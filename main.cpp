...
int count,n=0;
char firstArray[10][100];
char followArray[10][100];
int m=0;
...
int main()
{
    ...
    // Reading productions from file
    std::ifstream inputFile("../grammar.txt");
    ...
    count = 0;
    while(inputFile >> production[count]) {
        std::cout<<production[count]<<std::endl;
        count++;
    }
    std::cout << "Entered " << count << " lines of the grammar\n";
    ...
    for(k=0;k<count;k++)
    {
        ...
        for(i=0+jm;i<n;i++){
            ...
            for(lark=0;lark<point2;lark++){
                if (first[i] == firstArray[point1][lark]){
                    chk = 1;
                    break;
                }
            }
            if(chk == 0){
                printf("%c, ",first[i]);
                firstArray[point1][point2++] = first[i];
            }
        }
        printf("}\n");
        jm=n;
    ...
    for(e=0;e<count;e++)
    {
        ...
        for(i=0+km;i<m;i++){
            int lark, chk = 0;
            for(lark=0;lark<point2;lark++){
                if (f[i] == followArray[point1][lark]){
                    chk = 1;
                    break;
                }
            }
        ...
        while(production[ap][k] != '\0'){
            if(!isupper(production[ap][k])){
                ...
                for(zap=0;zap<count;zap++){
                    if(firstArray[zap][0] == production[ap][k]){
                        for(tuna=1;tuna<100;tuna++){
                            if(firstArray[zap][tuna] != '!'){
                                tem[ct++] = firstArray[zap][tuna];
                            }
                            else
                                break;
                        }
